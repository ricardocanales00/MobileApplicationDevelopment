...
class AgregarReporteView: UIViewController, UIPickerViewDelegate, UIPickerViewDataSource, UIImagePickerControllerDelegate, UINavigationControllerDelegate, CLLocationManagerDelegate {
    ...
    @IBAction func agregarReporte(_ sender: Any) {
        ...
        if (nombreTF.text != "" && lugarTF.text != "" && descripcionTF.text != "-" && lugar2TF.text != ""){
            let ref = self.db.collection("reporte").addDocument(data: ["nombre": nombreTF.text! , "terminado": false , "lugar" : String(lugarTF.text! + "-" + lugar2TF.text!), "descripcion" : descripcionTF.text, "fecha" : fechaTF.text!, "prioridad": prioridadSlider.value])
            
            
            if let data = imagen.image?.jpegData(compressionQuality: 00.5){
                let url = String("imagenes/" + ref.documentID + ".jpeg")
                storage.child(url).putData(data, metadata: nil, completion: {_, error in
