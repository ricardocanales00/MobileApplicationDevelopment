...
class EditarReporteViewController: UIViewController, UIPickerViewDelegate, UIPickerViewDataSource, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    override func viewDidLoad() {
        ...
        prioridadSlider.value = Float(reporteInicial.prioridad)
    ...
    */
    ...
    @IBAction func actualizarInfo(_ sender: Any) {
        if (nombreTF.text != "" && lugarTF.text != ""  && lugarTF.text != "-" && lugar2TF.text != ""){
            let reporteEditado = Reporte(nombre: nombreTF.text!, lugar: lugarTF.text! + "-" + lugar2TF.text!, fecha: reporteInicial.fecha, descripcion: descripcionTF.text!, prioridad: Double(prioridadSlider.value), key: reporteInicial.key, latitud: reporteInicial.latitud, longitud: reporteInicial.longitud)
            
            self.db.collection("reporte").document(reporteInicial.key).updateData(["lugar": lugarTF.text! + "-" + lugar2TF.text!, "descripcion": descripcionTF.text!, "prioridad": Double(prioridadSlider.value), "nombre": nombreTF.text!])
