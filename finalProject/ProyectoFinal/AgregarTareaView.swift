    ...
    @IBAction func agregarTarea(_ sender: Any) {
        if (nombreTF.text != "" && tipoTF.text != "" && usuarioTF.text != ""){
            ...
            if (arregloDescripcion.count == 0){
                ...
                let tarea = Tarea(nombre: nombreTF.text!, tipo: tipoTF.text!, usuario: usuarioTF.text!, fecha: fechaTF.text!, descripcion: descripcionTarea)
                
                listaTareas.append(tarea)
                
                delegate.agregarTarea(listaTarea: listaTareas)
