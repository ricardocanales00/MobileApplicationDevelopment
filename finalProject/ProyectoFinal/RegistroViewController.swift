            ...
            Auth.auth().createUser(withEmail: email, password: pass) { (result, error) in
                if let result = result, error == nil{
                    ...
                    Auth.auth().currentUser?.sendEmailVerification { (error) in
                        self.db.collection("users").document(Auth.auth().currentUser?.uid ?? " ").setData(["usuario":self.usernameTF.text ?? "","correo": email, "telefono":self.phoneTF.text ?? "", "cargo": self.cargoTF.text ?? "", "verificacion": false, "admin": false])
